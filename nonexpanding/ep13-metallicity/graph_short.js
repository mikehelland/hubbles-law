function metalGraph(graph, points, refs) {
    ...
    function drawGraph() {
        ...
        ctxB.fillText("Lookback Time (billion years)", graph.width / 2 + offset / 2, graph.height  - 50)
        ctxB.fillText("Redshift (z)", graph.width / 2 + offset / 2, 70)
        ...
        ctxB.save()
        ctxB.font = "22px sans-serif"

        ctxB.translate(-offset, -height / 2 + offset / 2)
        ctxB.rotate(-Math.PI/2)
        ctxB.textAlign = "center"
        ctxB.fillText("12 + log(O/H)", 0, 62)

        //ctxB.translate(0, 985)
        ctxB.fillStyle = "white"
        ctxB.fillText("BIG BANG", 0, graph.width - 10)
        ctxB.restore()
    ...
    var iPoints = 0
    function drawNext(yearDiv, list) {
        if (iPoints >= points.length) {
            return
        }

        while (!points[iPoints].data) {
            iPoints++
        }

        var point = points[iPoints]
        yearDiv.innerHTML = refs[point.ref].year
        drawRef(point.ref)

        li = document.createElement("li")
        li.innerHTML = `(${refs[point.ref].year}) ${point.name} <b>${refs[point.ref].title}</b>`                
        ol.innerHTML = ''
        ol.appendChild(li)
    ...
    return {draw: drawGraph, drawNextLocal, drawJWST, drawDESI, 
        ...
        drawLine, drawLine2, drawLine3, drawLine4, drawNext}
}
