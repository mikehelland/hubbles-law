...
var animClocksHandle
function animateClocks() {
    tStart = Date.now()
    animClocksHandle = setInterval(() => {
...
function drawTicks(ctx, mode) {
    ...
    ticks.forEach(tick => {
        ...
        if (!tick.hit && tr - tick.i * 100 > tick.t) {
            tick.hit = tr
        }
