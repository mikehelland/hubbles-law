(function () {
    var runButton = document.getElementById("run-flatness")
    var running 
    runButton.onclick = e => {
        if (!running) {
            runButton.innerHTML = "Stop"
            running = setInterval(run, 1000/60)
        }
        else {
    ...
    var canvas = document.getElementById("flat1")
    var ctx = canvas.getContext("2d")
    ctx.translate(canvas.width / 2, canvas.height / 2)

    var canvas2 = document.getElementById("flat2")
